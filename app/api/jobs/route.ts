...
export async function GET(request: NextRequest) {
  ...
  const searchParams = new URLSearchParams({ app_id: appId, app_key: appKey, results_per_page: "20", what: keyword, sort_by: "date", "content-type": "application/json" });
  searchParams.set("where", location);

  try {
    const response = await fetch(`https://api.adzuna.com/v1/api/jobs/${country}/search/${page}?${searchParams}`, { next: { revalidate: 60 } });
    if (!response.ok) {
    ...
    const data = (await response.json()) as { count?: number; results?: AdzunaJob[] };
